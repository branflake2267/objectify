...
public class IndexingEmbeddedTests extends TestBase {
  ...
  public static class EmbeddedIndexedPojo {
    ...
    @Unindex
    private boolean aProp = true;

    @Index
    @Embed
    private DefaultIndexedPojo[] indexed = {new DefaultIndexedPojo()};
    @Unindex
    @Embed
    private DefaultIndexedPojo[] unindexed = {new DefaultIndexedPojo()};
    @Embed
    private DefaultIndexedPojo[] def = {new DefaultIndexedPojo()};
  ...
  public void testEmbeddedGraph() throws Exception {
    /*
     * one.twoClass.bar = "A" one.twoField.bar = "A" one.foo = "1" id = ? prop = "A"
     */
    TestObjectify ofy = this.fact.begin();
