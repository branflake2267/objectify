...
/**
 * These are fairly pathological cases WRT nulls in embedded collections. They're really hard to solve, possibly
 * impossible. @Embed collections have a lot of limitations; really the whole approach is hackish. It will eventually be
 * deprecated in favor of a solution based on EmbeddedEntity, so these bugs probably won't get fixed.
 */
public class EmbeddedNullBroken extends TestBase {
  ...
  private FooBar createFooBar() {
    ...
    /*
     * @formatter:off Here is the root of the issue. foos is an array where the first element is non-null but the
     * remaining are null. In that case the underlying appengine Entity stores values like this
     * 
     * fooBar.foos.bar.aField = [aField] fooBar.foos.bar.bField = [<someRandomNumber>] fooBar.foos.bar = [null, null,
     * null, null] <=== Note there are 4 here, not 5
     * 
     * I would think that the expected behavior would be something like
     * 
     * fooBar.foos.bar.aField = [aField, aField, aField, aField, aField] fooBar.foos.bar.bField = [1,2,3,4,5]
     * 
   ...
   * 
   * I just ran across an anomaly (hesitate to call it a bug yet) re: nested @Embedded. Consider the following:
   * 
   * @Entity class Form {
   * @Embedded Approval[] approvals; }
   * 
   *           class Approval {
   * @Embedded AddressBook addressBook;
   * 
   *           String approvedBy; }
   * 
   *           class AddressBook {
   * @Serialized Group[] groups;
   * @Serialized Profile[] profiles; }
   * 
   *             class Group { String name; }
   * 
   *             class Profile { String name; }
   * 
   *             Assume I create a Form with 3 Approval objects in the approvals array. Each Approval has a non-null
   *             value for approvedBy. Approvals[0] and Approvals[2] have non-null AddressBook value for addressBook.
   *             Approvals[1] has a null value for addressBook.
   * 
   *             Now I put the Form.
   * 
   *             The flattened structure in the data store for Form will have the field 'approvals.addressBook' with a
   *             null value in addition to the 'approvals.addressBook.groups' and 'approvals.addressBook.profiles'
   *             fields that were expected.
   * 
   *             When I read the Form from the data store I will only get 2 Approval objects in the approvals array and
   *             the data in those 2 Approval objects will be a weird Frankenstein of values from the original 3
   *             Approval objects I put into the data store.
   * 
   *             If the Form is saved with non-null values in the addressBook field fro all 3 Approval objects, no
   *             'approvals.addressBook' field is created in the data store and when read, all 3 Approval objects are
   *             present with no problems.
   */
