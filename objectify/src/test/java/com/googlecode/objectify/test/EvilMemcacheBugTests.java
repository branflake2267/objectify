...
public class EvilMemcacheBugTests extends TestBase {
  ...
  static class SimpleEntity {
    ...
    String id;
    @Parent
    Key<SimpleParent> simpleParentKey;
    String foo = "bar";
  ...
  public void testMoreSophisticatedInAndOutOfTransaction() throws Exception {
    ...
    try {
      ...
      if (txnOfy.getTxn().isActive()) txnOfy.getTxn().rollback();
    }
  ...
  public void testEntityKeys() throws Exception {
    ...
    assert new String(MemcacheSerialization.makePbKey(entA1.getKey())).equals(new String(
        MemcacheSerialization.makePbKey(childKeyA)));
    assert new String(MemcacheSerialization.makePbKey(entA2.getKey())).equals(new String(
        MemcacheSerialization.makePbKey(childKeyA)));
    ...
    // This works
    assert new String(MemcacheSerialization.makePbKey(entB1.getKey())).equals(new String(
        MemcacheSerialization.makePbKey(childKeyB)));
    ...
    // Update: This succeeds! As of SDK 1.6.0 this has been fixed. The Objectify workaround (stringifying keys) has been
    // removed.
    assert new String(MemcacheSerialization.makePbKey(entB2.getKey())).equals(new String(
        MemcacheSerialization.makePbKey(childKeyB)));
  }
