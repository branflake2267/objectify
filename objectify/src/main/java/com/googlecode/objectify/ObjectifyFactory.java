...
/**
 * <p>
 * Factory which allows us to construct implementations of the Objectify interface. Just call {@code begin()}.
 * </p>
 * 
 * <p>
 * Note that unlike the DatastoreService, there is no implicit transaction management. You either create an Objectify
 * without a transaction (by calling {@code begin()} or you create one with a transaction (by calling
 * {@code beginTransaction()}. If you create an Objectify with a transaction, you should use it like this:
 * </p>
 * <code><pre>
 ...
 * 
 * <p>
 * ObjectifyFactory is designed to be subclassed; much default behavior can be changed by overriding methods. In
 * particular, see createObjectify(), construct(), getAsyncDatastoreService().
 * </p>
 * 
...
public class ObjectifyFactory {
  ...
  protected EntityMemcache entityMemcache = new EntityMemcache(MEMCACHE_NAMESPACE, new CacheControlImpl(this),
      this.memcacheStats);

  /**
   * <p>
   * Construct an instance of the specified type. Objectify uses this method whenever possible to create instances of
   * entities, condition classes, or other types; by overriding this method you can substitute Guice or other dependency
   * injection mechanisms. By default it constructs with a simple no-args constructor.
   * </p>
   */
  ...
  /**
   * <p>
   * Construct a collection of the specified type and the specified size for use on a POJO field. You can override this
   * with Guice or whatnot.
   * </p>
   * 
   * <p>
   * The default is to call construct(Class), with one twist - if a Set, SortedSet, or List interface is presented,
   * Objectify will construct a HashSet, TreeSet, or ArrayList (respectively). If you override this method with
   * dependency injection and you use uninitialized fields of these interface types in your entity pojos, you will need
   * to bind these interfaces to concrete types.
   * </p>
   */
   ...
   * 
   * <p>
   * The default is to call construct(Class), with one twist - if a Map or SortedMap List interface is presented,
   * Objectify will construct a HashMap or TreeMap (respectively). If you override this method with dependency injection
   * and you use uninitialized fields of these interface types in your entity pojos, you will need to bind these
  ...
  /**
   * Get an AsyncDatastoreService facade appropriate to the options. All Objectify datastore interaction goes through an
   * AsyncDatastoreService. This might or might not produce a CachingAsyncDatastoreService.
   * 
  ...
  /**
   * This is the beginning of any Objectify session. It creates an Objectify instance with the default options, unless
   * you override this method to alter the options. You can also override this method to produce a wholly different
   * Objectify implementation (possibly using ObjectifyWrapper).
   * 
   * <p>
   ...
   * 
   * <p>
   * Note that when using Objectify you will almost never directly call this method. Instead you should call the static
   * ofy() method on ObjectifyService.
   * </p>
   * 
  ...
  /**
   * <p>
   * All POJO entity classes which are to be managed by Objectify must be registered first. This method must be called
   * in a single-threaded mode sometime around application initialization.
   * </p>
   * 
   * <p>
  ...
  /**
   * Allocates a single id from the allocator for the specified kind. Safe to use in concert with the automatic
   * generator. This is just a convenience method for allocateIds().
   * 
   ...
   * 
   * @param parentKeyOrEntity must be a legitimate parent for the class type. It need not point to an existent entity,
   *          but it must be the correct type for clazz.
   * @param clazz must be a registered entity class with a Long or long id field, and a parent key of the correct type.
  ...
  /**
   * Preallocate a contiguous range of unique ids within the namespace of the specified entity class. These ids can be
   * used in concert with the normal automatic allocation of ids when put()ing entities with null Long id fields.
   * 
   * @param clazz must be a registered entity class with a Long or long id field.
   ...
   * These ids can be used in concert with the normal automatic allocation of ids when put()ing entities with null Long
   * id fields.
   * 
   * @param parentKeyOrEntity must be a legitimate parent for the class type. It need not point to an existent entity,
   *          but it must be the correct type for clazz.
  ...
  /**
   * Allocates a user-specified contiguous range of unique IDs, preventing the allocator from giving them out to
   * entities (with autogeneration) or other calls to allocate methods. This lets you specify a specific range to block
   * out (for example, you are bulk-loading a collection of pre-existing entities). If you don't care about what id is
   * allocated, use one of the other allocate methods.
   */
  ...
  /**
   * <p>
   * Gets the master list of all registered TranslatorFactory objects. By adding Translators, Objectify can process
   * additional field types which are not part of the standard GAE SDK. <b>You must add translators *before* registering
   * entity pojo classes.</b>
   * </p>
   * 
