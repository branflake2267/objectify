 ...
 * 
 * <p>
 * Note that the methods might or might not throw runtime exceptions related to datastore operations;
 * ConcurrentModificationException, DatastoreTimeoutException, DatastoreFailureException, and
 * DatastoreNeedIndexException. Some Refs hide asynchronous operations that could throw these exceptions.
 * </p>
 * 
...
abstract public class Ref<T> implements Serializable, Comparable<Ref<T>> {
  ...
  public static <T> Ref<T> create(Key<T> key) {
    if (key == null) throw new NullPointerException("Cannot create a Ref from a null key");
  ...
  /**
   * Nearly identical to get() but conforms to JavaBean conventions and returns null instead of throwing
   * IllegalStateException if uninitialized. This is convenient for use in a JSON converter or an expression language.
   * 
  ...
  /**
   * Same as key() but conforms to JavaBeans conventions in case this is being processed by a JSON converter or
   * expression language.
   */
