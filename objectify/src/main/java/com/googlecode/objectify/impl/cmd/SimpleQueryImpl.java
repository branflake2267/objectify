...
/**
 * Base for command classes that include methods for defining a query (filter, order, limit, etc). Does not include the
 * methods for executing a query.
 * 
...
abstract class SimpleQueryImpl<T> implements SimpleQuery<T> {
  ...
  /**
   * Create an initial query object; for a real FindType this will have a class, otherwise it will be generic. For the
   * real QueryImpl itself this is a clone() operation.
   */
