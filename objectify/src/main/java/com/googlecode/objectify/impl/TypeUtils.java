...
public class TypeUtils {
  ...
  /**
   * Gets a constructor that has the specified types of arguments. Throw an IllegalStateException if the class does not
  ...
  public static boolean isOfInterest(Field field) {
    return !field.isAnnotationPresent(Ignore.class) && ((field.getModifiers() & NOT_SAVEABLE_MODIFIERS) == 0)
        && !field.isSynthetic();
  }
  ...
  public static boolean isOfInterest(Method method) {
    for (Annotation[] annos : method.getParameterAnnotations())
      if (getAnnotation(AlsoLoad.class, annos) != null) return true;
   ...
   * 
   * @return the fields we load and save, including @Id and @Parent fields. All fields will be set accessable and
   *         returned in order starting with superclass fields.
   */
  ...
  private static void getProperties(ObjectifyFactory fact, Class<?> clazz, List<Property> good, Class<?> topClass) {
    ...
    for (Field field : clazz.getDeclaredFields())
      if (isOfInterest(field)) good.add(new FieldProperty(fact, topClass, field));

    for (Method method : clazz.getDeclaredMethods())
      if (isOfInterest(method)) good.add(new MethodProperty(method));
  }
  ...
  public static <A extends Annotation> A getAnnotation(Class<A> annotationType, Annotation[] annotations) {
    for (Annotation anno : annotations)
      if (annotationType.isAssignableFrom(anno.getClass())) return (A) anno;
