 ...
 * 
 * <p>
 * A node can have a map, a list, or a property value. However, these are not exclusive; because of the entity storage
 * structure a node can have a map and a property value at the same place. Lists, however, stand alone.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * This is roughly similar to Jackson's JsonNode. One major difference is that there is not a class hierarchy of node
 * types; this is because nodes can actually switch types during tree reprocessing.
 * </p>
 * 
...
public class Node implements Iterable<Node> {
  ...
  /**
   * A property value could appear at any point in the entity tree. This is because we might attach denormalized data
   * for the value at some point, and that data will likely be in a map underneath.
   */
  ...
  /**
   * Gets the node at the index of the list, making sure that a node exists there. Assumes that the indexes are always
   * fed in order.
   */
  ...
  public String toString() {
    ...
    if (hasPropertyValue()) bld.append('{').append(getPropertyValue()).append('}');

    if (hasPropertyValue() && !isEmpty()) bld.append('+');

    if (!isEmpty()) if (lazyMap != null)
