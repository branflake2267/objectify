...
/**
 * Knows how to map between datastore Entity objects and your typed POJO objects. An instance of this class knows how to
 * perform mapping of a specific subclass.
 * 
...
public class ConcreteEntityMetadata<T> implements EntityMetadata<T> {
  ...
  /**
   * Recursive function which walks up the superclass hierarchy looking for lifecycle-related methods (@OnSave and
   * @OnLoad).
   */
        ...
        for (int i = 0; i < ptypes.length; i++)
          if (ptypes[i] != Objectify.class && ptypes[i] != LoadContext.class && ptypes[i] != SaveContext.class)
            throw new IllegalStateException(
                "@OnSave and @OnLoad methods can only have parameters of type Objectify, LoadContext, or SaveContext");

        if (method.isAnnotationPresent(OnSave.class)) {
          if (this.onSaveMethods == null) this.onSaveMethods = new ArrayList<Method>();

          this.onSaveMethods.add(method);
        }

        if (method.isAnnotationPresent(OnLoad.class)) {
          if (this.onLoadMethods == null) this.onLoadMethods = new ArrayList<Method>();
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.EntityMetadata#toObject(com.google.appengine.api.datastore.Entity,
   ...
   */
  private void invokeLifecycleCallbacks(List<Method> callbacks, Object pojo, Objectify ofy, LoadContext lctx,
      SaveContext sctx) {
    try {
      if (callbacks != null) for (Method method : callbacks)
