...
/**
 * Represents one "batch" of loading. Get a number of Result<?> objects, then execute(). Some work is done right away,
 * some work is done on the first get(). There might be multiple rounds of execution to process all the @Load groups,
 * but that is invisible outside this class.
 * 
...
public class LoadEngine {
  ...
  /**
   * Each round in the series of fetches required to complete a batch. A round executes when the value is obtained (via
   * now()) for a Result that was created as part of this round. When a round executes, a new round is created.
   */
  class Round {
    ...
    public <T> Result<T> get(final Key<T> key) {
      ...
      if (sv == null) {
        if (log.isLoggable(Level.FINEST)) log.finest("Adding to round (session miss): " + key);
        ...
        if (log.isLoggable(Level.FINEST)) log.finest("Adding to round (session hit): " + key);
      }
    ...
    public void execute() {
      if (needsExecution()) {
        if (log.isLoggable(Level.FINEST)) log.finest("Executing round: " + pending);
        ...
        translated = new ResultCache<Map<Key<?>, Object>>() {
          ...
          /**
           * We need to execute the done() after the translated value has been set, otherwise we can produce an infinite
           * recursion problem.
           */
          @Override
  ...
  public LoadEngine(LoaderImpl loader) {
    ...
    if (log.isLoggable(Level.FINEST)) log.finest("Starting load engine with groups " + loader.getLoadGroups());
  }
  ...
  /**
   * Gets the result, possibly from the session, putting it in the session if necessary. Also will recursively prepare
   * the session with @Load parents as appropriate.
   */
  ...
  /**
   * Create a Ref for the key, and maybe initialize the value depending on the load annotation and the current state of
   * load groups. If appropriate, this will also register the ref for upgrade.
   * 
  ...
  /**
   * Stuffs an Entity into a place where values in the round can be obtained instead of going to the datastore. Called
   * by non-hybrid queries to add results and eliminate batch fetching.
   */
