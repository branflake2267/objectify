...
public class TranslatableProperty<T> {
  ...
  /**
   * Sets the property on the pojo to the value. However, sensitive to the value possibly being a Result<?> wrapper, in
   * which case it enqueues the set operation until the loadcontext is done.
   */
  ...
  private Node getChild(Node parent, Property prop) {
    ...
    for (String name : prop.getLoadNames()) {
      ...
      if (child != null && child2 != null)
        throw new IllegalStateException("Collision trying to load field; multiple name matches for '" + prop.getName()
            + "' at '" + child.getPath() + "' and '" + child2.getPath() + "'");
