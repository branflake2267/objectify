...
/**
 * <p>
 * A translator knows how convert between POJO objects and the EntityNode tree structure that Objectify can persist. In
 * principle, this is similar to how a JSON object mapper works. Each node in the object graph corresponds to a node in
 * the EntityNode graph.
 * </p>
 * 
 * <p>
 * Translators are composed of other translators; through a chain of these a whole entity object is assembled or
 * disassembled.
 * </p>
 * 
...
public interface Translator<T> {
   ...
   * 
   * <p>
   * There is one special return value: If a Result<?> is returned, the content of the Result will be used instead, but
   * delayed until ctx.done() is called. This happens at the end of a "round" of load operations and is the magic trick
   * that makes populating entity references work efficiently.
   * </p>
   * 
  ...
  /**
   * Translates the pojo into an EntityNode format. Note that all stored values (even terminal properties) are
