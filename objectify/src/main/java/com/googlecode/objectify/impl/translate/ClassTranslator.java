...
/**
 * Translator which knows what to do with a whole class. This is used by the EmbedClassTranslatorFactory and also
 * subclassed to produce an EntityClassTranslator, which is managed specially.
 * 
...
public class ClassTranslator<T> extends MapNodeTranslator<T> {
  ...
  public ClassTranslator(Class<T> clazz, Path path, CreateContext ctx) {
    ...
    if (!Modifier.isAbstract(clazz.getModifiers())) {
      try {
        ...
        throw new IllegalStateException("Unable to construct an instance of " + clazz.getName()
            + "; perhaps it has no suitable constructor?", ex);
      }
    }
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.translate.MapNodeTranslator#loadMap(com.googlecode.objectify.impl.Node,
   * com.googlecode.objectify.impl.translate.LoadContext)
   */
  @Override
  protected T loadMap(Node node, LoadContext ctx) {
    if (log.isLoggable(Level.FINEST)) log.finest(LogUtils.msg(node.getPath(), "Instantiating a " + clazz.getName()));
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.translate.MapNodeTranslator#saveMap(java.lang.Object,
   * com.googlecode.objectify.impl.Path, boolean, com.googlecode.objectify.impl.translate.SaveContext)
   */
  @Override
