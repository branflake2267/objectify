...
/**
 * <p>
 * Translator which manages an expando-style Map. The key must be String, which becomes a normal path element in the
 * entity. The value can be anything that goes into a Collection; a basic type, an embedded type, a reference, a null,
 * etc. Pretty much the same attribute rules apply as apply to Collections.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * However, Maps are not list structures, so you don't have the same restriction on embedding; you can put maps inside
 * of maps and lists inside of maps.
 * </p>
 * 
...
public class MapTranslatorFactory implements TranslatorFactory<Map<String, Object>> {
  abstract public static class MapMapNodeTranslator extends MapNodeTranslator<Map<String, Object>> {
    ...
    /**
     * Load into an existing map; allows us to recycle map instances on entities, which might have exotic concrete types
     * or special initializers (comparators, etc).
     * 
  ...
  public Translator<Map<String, Object>> create(Path path, final Property property, Type type, CreateContext ctx) {
    ...
    return new MapMapNodeTranslator() {
      ...
      public Map<String, Object> loadMapIntoExistingMap(Node node, LoadContext ctx, Map<String, Object> map) {
        ...
        for (Node child : node) {
          String mapKey = child.getPath().getSegment();
      ...
      protected Node saveMap(Map<String, Object> pojo, Path path, boolean index, SaveContext ctx) {
        ...
        for (Map.Entry<String, ?> entry : pojo.entrySet()) {
          if (entry.getKey() == null) throw new IllegalArgumentException("Map keys cannot be null");
