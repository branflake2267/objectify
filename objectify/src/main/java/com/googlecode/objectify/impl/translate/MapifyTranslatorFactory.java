...
/**
 * <p>
 * This takes a collection and converts it to a map by letting you select out the key value using a class of your own
 * devising. All the rules for collections normally apply (ie, you can't have collections inside of collections) but
 * otherwise this works just like a map. The values will be written to the collection, not the keys.
 * </p>
 * 
...
public class MapifyTranslatorFactory implements TranslatorFactory<Map<Object, Object>> {
  ...
  public Translator<Map<Object, Object>> create(Path path, final Property property, Type type, CreateContext ctx) {
    ...
    if (mapify == null) return null;

    @SuppressWarnings("unchecked")
    final Class<? extends Map<?, ?>> mapType = (Class<? extends Map<?, ?>>) GenericTypeReflector.erase(type);

    if (!Map.class.isAssignableFrom(mapType)) return null; // We might be here processing the component type of the
                                                           // mapify map!
