...
/**
 * <p>
 * Helper which expects a list node in the data structure and throws an exception one is not found. Also handles
 * skipping when a null list is found.
 * </p>
 * 
...
abstract public class ListNodeTranslator<T> extends AbstractTranslator<T> {
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.translate.AbstractTranslator#loadAbstract(com.googlecode.objectify.impl.Node,
  ...
  final public T loadAbstract(Node node, LoadContext ctx) {
    ...
    if (node.hasPropertyValue() && node.getPropertyValue() == null) throw new SkipException();

    if (!node.hasList()) node.getPath().throwIllegalState("Expected list structure but found " + node);
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.translate.AbstractTranslator#saveAbstract(java.lang.Object,
  ...
  final public Node saveAbstract(T pojo, Path path, boolean index, SaveContext ctx) {
    ...
    if (pojo == null) throw new SkipException();
