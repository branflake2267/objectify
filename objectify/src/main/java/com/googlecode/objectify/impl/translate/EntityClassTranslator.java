...
/**
 * <p>
 * Translator which can maps the root of an entity. There is no factory associated with this; you just instantiate one
 * as necessary.
 * </p>
 * 
...
public class EntityClassTranslator<T> extends ClassTranslator<T> implements KeyMetadata<T> {
  ...
  protected void foundTranslatableProperty(TranslatableProperty<Object> tprop) {
    ...
    if (prop.getAnnotation(Id.class) != null) {
      ...
      if ((prop.getType() != Long.class) && (prop.getType() != long.class) && (prop.getType() != String.class))
        throw new IllegalStateException("@Id field '" + prop.getName() + "' in " + clazz.getName()
            + " must be of type Long, long, or String");
      ...
      if (!isAllowedParentFieldType(prop.getType()))
        throw new IllegalStateException("@Parent fields must be Ref<?>, Key<?>, or datastore Key. Illegal parent: "
            + prop);
  ...
  private boolean isAllowedParentFieldType(Type type) {
    ...
    return com.google.appengine.api.datastore.Key.class.isAssignableFrom(erased) || Key.class.isAssignableFrom(erased)
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.KeyMetadata#setKey(java.lang.Object, com.google.appengine.api.datastore.Key,
  ...
  public void setKey(T pojo, com.google.appengine.api.datastore.Key key, LoadContext ctx) {
    if (!clazz.isAssignableFrom(pojo.getClass()))
      throw new IllegalArgumentException("Trying to use metadata for " + clazz.getName() + " to set key of "
  ...
  public com.google.appengine.api.datastore.Entity initEntity(T pojo) {
    ...
    if (id == null)
      if (isIdNumeric()) {
        if (log.isLoggable(Level.FINEST)) log.finest("Getting parent key from " + pojo);
  ...
  public com.google.appengine.api.datastore.Key getRawKey(T pojo) {
    ...
    if (log.isLoggable(Level.FINEST)) log.finest("Getting key from " + pojo);

    if (!clazz.isAssignableFrom(pojo.getClass()))
      throw new IllegalArgumentException("Trying to use metadata for " + clazz.getName() + " to get key of "
          + pojo.getClass().getName());
  ...
  public boolean shouldLoadParent(Set<Class<?>> enabledGroups) {
    if (this.parentMeta == null) return false;
