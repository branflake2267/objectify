...
/**
 * <p>
 * An implementation of Ref that works like a proxy. We can't create dynamic proxies on abstract classes and we don't
 * want to suck in a bytecode manipulator dependency so we have to create this by hand. No big deal.
 * </p>
 * 
...
abstract public class ProxyRef<T> extends Ref<T> {
  ...
  private final Ref<T> getRef() {
    if (this.cached == null) this.cached = ref();
