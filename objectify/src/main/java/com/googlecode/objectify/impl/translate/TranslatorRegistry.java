 ...
 * Essentially acts as an aggregator for all the TranslatorFactory objects.
 * </p>
 * 
 * <p>
 * When Objectify arranges a translator for a type at registration time, it runs through the available TranslatorFactory
 * instances one at time looking for one that will provide a Translator. The first one found is kept and used during
 * runtime assembly and disassembly of entities.
 * </p>
 * 
...
public class TranslatorRegistry {
  ...
  public TranslatorRegistry(ObjectifyFactory fact) {
    ...
    this.translators.add(new EmbedClassTranslatorFactory<Object>()); // AFTER the various collections so we only process
                                                                     // the content
  ...
  /**
   * <p>
   * Add a new translator to the list. Translators are added in order after most of the "plumbing" translators
   * (collections, arrays, maps, serialize, embeds, references) but before any of the standard value conversions like
   * String, Number, Key, Enum, SqlDate, TimeZone, etc.
   * </p>
   * 
   * <p>
