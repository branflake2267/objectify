...
/**
 * Knows how to convert Strings. Datastore representation might be String or it might be Text. Will work with anything
 * that's in the datastore just by calling toString() on what we get back; convenient for converting between say Number
 * and the String representation, possibly dangerous otherwise.
 */
        ...
        if (value.length() > 500) {
          if (disallowConversion)
            path.throwIllegalState("Objectify cannot autoconvert Strings greater than 500 characters to Text within @Embed collections."
                + "  Use Text for the field type instead." + "  You tried to save: " + value);
