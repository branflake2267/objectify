...
/**
 * The interface by which POJOs and datastore Entity objects are translated back and forth. Subclasses implement
 * specific mapping, including polymorphic mapping.
 * 
...
public class PolymorphicEntityMetadata<T> implements EntityMetadata<T> {
  ...
  static class SubclassInfo<V> {
    ...
    /**
     * The discriminators that will be indexed for this subclass. Empty for the base class or any subclasses for which
     * all discriminators are unindexed.
     */
    ...
    public void addIndexedDiscriminators(Class<?> clazz) {
      if (clazz.isAnnotationPresent(com.googlecode.objectify.annotation.Entity.class)) return;
  ...
  /**
   * If the entity is null, return the metadata for the root entity of the polymorphic hierarchy. This will have the
   * effect of making cache misses use the @Cached annotation of the @Entity.
   * 
   * @return the concrete entity metadata given the discriminator info
   */
  private EntityMetadata<? extends T> getConcrete(Entity ent) {
    if (ent == null) return this.base.metadata;

    String discriminator = (String) ent.getProperty(DISCRIMINATOR_PROPERTY);

    if (discriminator == null) return this.base.metadata;
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.EntityMetadata#load(com.google.appengine.api.datastore.Entity,
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.EntityMetadata#save(java.lang.Object,
  ...
  public Entity save(T pojo, SaveContext ctx) {
    ...
    if (info.discriminator != null) ent.setUnindexedProperty(DISCRIMINATOR_PROPERTY, info.discriminator);
