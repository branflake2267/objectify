...
abstract public class AbstractProperty implements Property {
  ...
  public AbstractProperty(String name, Annotation[] annotations, Object thingForDebug) {
    ...
    if (this.getAnnotation(IgnoreLoad.class) == null) nameSet.add(name);

    // Now any additional names, either @AlsoLoad or the deprecated @OldName
    AlsoLoad alsoLoad = this.getAnnotation(AlsoLoad.class);
    if (alsoLoad != null) if (alsoLoad.value() == null || alsoLoad.value().length == 0)
    ...
    if (load != null) {
      ...
      if (load.unless().length > 0) loadUnlessGroups = load.unless();
    }
  }
  ...
  public boolean shouldLoad(Set<Class<?>> groups) {
    if (loadGroups == null) return false;

    if (loadGroups.length > 0 && !matches(groups, loadGroups)) return false;

    if (loadUnlessGroups != null && matches(groups, loadUnlessGroups)) return false;
  ...
  private boolean matches(Set<Class<?>> groups, Class<?>[] loadGroups) {
    for (Class<?> propertyGroup : loadGroups)
      for (Class<?> enabledGroup : groups)
        if (propertyGroup.isAssignableFrom(enabledGroup)) return true;
