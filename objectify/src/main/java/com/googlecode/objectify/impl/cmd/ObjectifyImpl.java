...
/**
 * Implementation of the Objectify interface. Note we *always* use the AsyncDatastoreService methods that use
 * transactions to avoid the confusion of implicit transactions.
 * 
...
public class ObjectifyImpl implements Objectify, Cloneable {
  ...
  public Objectify consistency(Consistency value) {
    if (value == null) throw new IllegalArgumentException("Consistency cannot be null");
    ...
    while (true) {
      try {
        ...
        if (limitTries-- > 0) {
          ...
          if (log.isLoggable(Level.FINEST)) log.log(Level.FINEST, "Details of optimistic concurrency failure", ex);
        } else {
          throw ex;
        }
      }
  ...
  protected DatastoreServiceConfig createDatastoreServiceConfig() {
    ...
    if (deadline != null) cfg.deadline(deadline);
  ...
  /**
   * <p>
   * Translates the value of a filter clause into something the datastore understands. Key<?> goes to native Key,
   * entities go to native Key, java.sql.Date goes to java.util.Date, etc. It uses the same translation system that is
   * used for standard entity fields, but does no checking to see if the value is appropriate for the field.
   * </p>
   * 
   * <p>
  ...
  public Object makeFilterable(Object value) {
    ...
    if (value instanceof Iterable) {
      ...
      if (meta != null) {
        ...
        Translator<Object> translator = getFactory().getTranslators().create(Path.root(), NullProperty.INSTANCE,
            value.getClass(), new CreateContext(getFactory()));
