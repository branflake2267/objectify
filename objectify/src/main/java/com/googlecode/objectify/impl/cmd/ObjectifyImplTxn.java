...
/**
 * Implementation for when we start a transaction. Maintains a separate session, but then copies all data into the
 * original session on successful commit.
 * 
...
public class ObjectifyImplTxn extends ObjectifyImpl {
  ...
  public class TransactionImpl extends TransactionWrapper {
    /**
     * Operations which modify the session must be enlisted in the transaction and completed before the transaction
     * commits. This is so that the session reaches a consistent state before it is propagated to the parent session.
     */
  ...
  public ObjectifyImplTxn(ObjectifyImpl parent) {
    ...
    Future<Transaction> fut = createAsyncDatastoreService().beginTransaction(
        TransactionOptions.Builder.withXG(DatastoreIntrospector.SUPPORTS_XG));
   ...
   * (non-Javadoc)
   * 
   * @see com.googlecode.objectify.impl.cmd.ObjectifyImpl#execute(com.googlecode.objectify.TxnType,
   * com.googlecode.objectify.Work)
   */
  @Override
