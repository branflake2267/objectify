...
/**
 * This is a wrapper for a Path that makes it backwards; instead of going from tail to head, it has links from head to
 * tail. This is sometimes useful for forward traversal of a path.
 * 
...
public class ForwardPath {
  ...
  public static ForwardPath of(Path path) {
    ...
    if (path.getPrevious() == Path.root()) return next;
