 ...
 * 
 * <p>
 * Note that empty or null collections are not stored in the datastore, and null values for the collection field are
 * ignored when they are loaded from the Entity. This is because the datastore doesn't store empty collections, and
 * storing null fields will confuse filtering for actual nulls in the collection contents.
 * </p>
 * 
...
public class CollectionTranslatorFactory implements TranslatorFactory<Collection<Object>> {
  abstract public static class CollectionListNodeTranslator extends ListNodeTranslator<Collection<Object>> {
    ...
    /**
     * Load into an existing collection; allows us to recycle collection instances on entities, which might have exotic
     * concrete types or special initializers (comparators, etc).
     * 
     * @param coll can be null to trigger creating a new collection
     */
    abstract public Collection<Object> loadListIntoExistingCollection(Node node, LoadContext ctx,
  ...
  public Translator<Collection<Object>> create(Path path, final Property property, Type type, CreateContext ctx) {
    ...
    try {
      ...
      return new CollectionListNodeTranslator() {
        @Override
        @SuppressWarnings("unchecked")
        public Collection<Object> loadListIntoExistingCollection(Node node, LoadContext ctx,
        ...
        protected Node saveList(Collection<Object> pojo, Path path, boolean index, SaveContext ctx) {
          ...
          if (pojo == null || pojo.isEmpty()) throw new SkipException();
