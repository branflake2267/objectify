...
public class FieldProperty extends AbstractProperty {
  ...
  /**
   * @param examinedClass is the actual top level concrete class we are examining; the field might be declared on a
   *          superclass of this class so it's not the same as field.getDeclaringClass()
   */
  public FieldProperty(ObjectifyFactory fact, Class<?> examinedClass, Field field) {
    ...
    if (indexedAnn != null) this.indexConditions = ifGenerator.generateIfConditions(indexedAnn.value(), field);

    if (unindexedAnn != null) this.unindexConditions = ifGenerator.generateIfConditions(unindexedAnn.value(), field);
  ...
  private boolean matches(Object onPojo, If<?, ?>[] conditions) {
    ...
    for (int i = 0; i < conditions.length; i++) {
      ...
      if (cond.matchesValue(value)) return true;

      if (cond.matchesPojo(onPojo)) return true;
    }
