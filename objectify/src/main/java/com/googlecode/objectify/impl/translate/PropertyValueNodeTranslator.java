...
/**
 * <p>
 * Helper which expects a property value in the data structure and throws an exception if one is not found. Also handles
 * null checking.
 * </p>
 * 
...
abstract public class PropertyValueNodeTranslator<T> extends AbstractTranslator<T> {
  ...
  final public T loadAbstract(Node node, LoadContext ctx) {
    if (!node.hasPropertyValue()) node.getPath().throwIllegalState("Expected property value but found: " + node);

    if (node.getPropertyValue() == null) return null;
