...
class QueryImpl<T> extends SimpleQueryImpl<T> implements Query<T>, Cloneable {
  /**
   * Because we process @Load batches, we need to always work in chunks. So we should always specify a chunk size to the
   * query. This is the default if user does not specify an explicit chunk size.
   */
  ...
  void addOrder(String condition) {
    ...
    // Check for @Id or @Parent fields. Any setting adjusts the key order. We only enforce that they are both set the
    // same direction.
    if (this.classRestriction != null) {
      KeyMetadata<?> meta = Keys.getMetadataSafe(this.classRestriction);
  ...
  void setLimit(int value) {
    ...
    if (this.chunk == null) this.chunk = value;
  }
  ...
  public String toString() {
    ...
    bld.append(",ancestor=");
    if (this.actual.getAncestor() != null) bld.append(KeyFactory.keyToString(this.actual.getAncestor()));

    // We need to sort filters to make a stable string value
    FilterPredicate[] filters = this.actual.getFilterPredicates().toArray(
        new FilterPredicate[this.actual.getFilterPredicates().size()]);
    Arrays.sort(filters, new Comparator<FilterPredicate>() {
      ...
      public int compare(FilterPredicate o1, FilterPredicate o2) {
        ...
        if (o1.getValue() == null)
          ...
          return o1.getValue().toString().compareTo(o2.getValue().toString()); // not perfect, but probably as good as
                                                                               // we can do
      }
    });
    ...
    SortPredicate[] sorts = this.actual.getSortPredicates().toArray(
        new SortPredicate[this.actual.getSortPredicates().size()]);
    ...
    if (this.startAt != null) bld.append(",startAt=").append(this.startAt.toWebSafeString());

    if (this.endAt != null) bld.append(",endAt=").append(this.endAt.toWebSafeString());
  ...
  /**
   * Get an iterator over the keys. Not part of the public api, but used by QueryKeysImpl. Assumes that setKeysOnly()
  ...
  private QueryResultIterable<Ref<T>> refIterable() {
    ...
    if (!hasForcedHybrid) {
      ...
      if (hasMulti && hasNonKeyOrder) hybridize = false;
    }
  ...
  /**
   * @return a set of fetch options for the current limit, offset, and cursors, based on the default fetch options.
   *         There will always be options even if default.
   */
