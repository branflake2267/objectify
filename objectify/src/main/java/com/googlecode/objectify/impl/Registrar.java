 ...
 * 
 * <p>
 * There logic here is convoluted by polymorphic hierarchies. Entity classes can be registered in any particular order,
 * requiring some considerable care.
 * </p>
 * 
...
public class Registrar {
   ...
   * in a single-threaded mode sometime around application initialization. Re-registering a class has no effect.
   * </p>
   * 
  ...
  public <T> void register(Class<T> clazz) {
    ...
    if (this.byClass.containsKey(clazz)) return;
    ...
    if (clazz.isAnnotationPresent(Entity.class)) {
      ...
      if (cmeta.getCacheExpirySeconds() != null) this.cacheEnabled = true;
  ...
  /**
   * Recursively register classes in the hierarchy which have @EntitySubclass or @Entity. Stops when arriving at the
   * first @Entity. Safely handles classes that have already been registered, including @Entity classes that were
   * registered as non-polymorphic.
   * 
   * It's damn near impossible to get the generics to line up properly, so we just use rawtypes in this method. Forgive
   * me.
   */
  ...
  protected void registerPolymorphicHierarchy(String kind, Class clazz) {
    ...
    if (clazz == Object.class)
      throw new IllegalArgumentException(
          "An @EntitySubclass hierarchy must have an @Entity superclass (direct or indirect)");
