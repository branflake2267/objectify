...
public class QueryEngine {
   ...
   */
  public <T> QueryResultIterable<Ref<T>> query(com.google.appengine.api.datastore.Query query,
      final FetchOptions fetchOpts, final boolean hybridize) {
    ...
    if (hybridize) query = DatastoreUtils.cloneQuery(query).setKeysOnly();
  ...
  /**
   * Takes a keys-only iterable source, breaks it down into batches of a specific chunk size, and uses batch loading to
   * load the actual values. This makes @Cache and @Load annotations work.
   */
  protected class ChunkingToRefIterator<T> implements QueryResultIterator<Ref<T>> {
    ...
    public Ref<T> next() {
      ...
      if (!batchIt.hasNext()) this.advanceBatch();
    ...
    private void advanceBatch() {
      ...
      for (int i = 0; i < chunkSize; i++) {
        ...
        if (log.isLoggable(Level.FINEST)) log.finest("Query found " + ent.getKey());

        if (!hybrid && !keysOnly) loadEngine.stuff(ent);

        if (!keysOnly) loadEngine.loadRef(ref);
     ...
     * http://code
     * .google.com/p/googleappengine/source/browse/trunk/java/src/main/com/google/appengine/api/datastore/Cursor.java#70
     * 
     * Doing this for every result will definitely give you really bad performance. I have several yet to be implemented
     * ideas that would solve this problem (which you potentially could push me into prioritizing), but I believe you
     * can solve the performance problem today by saving the start_cursor an offset into the batch. Then you can
     * evaluate the real cursor on demand using
     * "query.asQueryResultIterator(withStartCursor(cursor).offset(n).limit(0)).getCursor()"
     */
    @Override
    public Cursor getCursor() {
      if (offsetIntoBatch == 0)
        ...
        return pq.asQueryResultIterator(
            FetchOptions.Builder.withStartCursor(baseCursor).offset(offsetIntoBatch).limit(0)).getCursor();
    }
