...
public class Session {
  ...
  public void add(Key<?> key, SessionValue<?> value) {
    if (log.isLoggable(Level.FINEST)) log.finest("Adding to session: " + key + " -> " + value.getResult());
  ...
  public void addAll(Session other) {
    if (log.isLoggable(Level.FINEST)) log.finest("Adding all values to session: " + other.map.keySet());
  ...
  /** */
  public void clear() {
    if (log.isLoggable(Level.FINEST)) log.finest("Clearing session");
