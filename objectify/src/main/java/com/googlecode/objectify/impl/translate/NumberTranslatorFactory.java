...
/**
 * <p>
 * Numbers are funky in the datastore. You can save numbers of any size, but they always retrieve as Long. For the hell
 * of it, we also handle String in the datastore by trying to parse it.
 * </p>
 * 
 * <p>
...
public class NumberTranslatorFactory implements TranslatorFactory<Number> {
  ...
  public ValueTranslator<Number, Object> create(Path path, Property property, Type type, CreateContext ctx) {
    ...
    if (!TypeUtils.isAssignableFrom(Number.class, clazz)) return null;
  ...
  /**
   * Coerces the value to be a number of the specified type; needed because all numbers come back from the datastore as
   * Long and this screws up any type that expects something smaller. Also does toString just for the hell of it.
   */
