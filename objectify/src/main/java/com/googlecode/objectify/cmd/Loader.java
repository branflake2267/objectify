 ...
 * 
 * <p>
 * At this point you can enable load groups with {@code group()}, start all-kinds queries by calling query-related
 * methods (see SimpleQuery), load entities by key or ref, or narrow your interest to a specific kind by calling
 * {@code type()}.
 * </p>
 * 
 * <p>
...
public interface Loader extends SimpleQuery<Object> {
  /**
   * <p>
   * Enables one or more fetch groups. This will cause any entity fields (or Ref fields) which are annotated with
   * @Load(XYZGroup.class) to be fetched along with your entities. The class definition can be any arbitrary class, but
   * inheritance is respected - if you have a {@code class Foo extends Bar}, then {@code group(Foo.class)} will cause
   * loading of all {@code @Load(Bar.class)} properties.
   * </p>
   * 
   * <p>
  ...
  /**
   * <p>
   * Restricts the find operation to entities of a particular type. The type may be the base of a polymorphic class
   * hierarchy. This is optional.
   * </p>
   * 
   * @param type is the type of entity (or entities) to retrieve, possibly a base class for a polymorphic hierarchy
   * @return the next step in the immutable command chain, which allows you to start a query or define keys for a batch
   *         get.
   */
  ...
  /**
   * <p>
   * Load a single entity ref. This starts an asynchronous fetch operation which will be available as ref.get().
   * </p>
   * 
   * <p>
   * Since fetching is asynchronous, <a
   * href="http://code.google.com/appengine/articles/handling_datastore_errors.html">datastore exceptions</a> (
   * {@code DatastoreTimeoutException}, {@code ConcurrentModificationException}, {@code DatastoreFailureException}) will
   * be thrown from {@code Ref<?>.get()}.
   * </p>
   * 
   ...
   * href="http://code.google.com/appengine/articles/handling_datastore_errors.html">datastore exceptions</a> (
   * {@code DatastoreTimeoutException}, {@code ConcurrentModificationException}, {@code DatastoreFailureException}) will
   * be thrown when the Map is accessed.
   * </p>
   * 
   ...
   * 
   * <p>
   * This is typically used to retrieve "unfetched" entities which have been returned as fields in other entities. These
   * unfetched entities will have their key fields (id/parent) filled but otherwise be uninitialized.
   * </p>
   * 
   * <p>
   * Since fetching is asynchronous, <a
   ...
   * <li>Ref<?> objects.</li>
   * <li>Registered entity instances which have id and parent fields populated. This is typically used to retrieve
   * "unfetched" entities which have been returned as fields in other entities. These unfetched entities will have their
   * key fields (id/parent) filled but otherwise be uninitialized.</li>
   * </ul>
   * 
   * <p>
   * Since fetching is asynchronous, <a
   ...
   * </p>
   * 
   * @param keysOrEntities defines a possibly heterogeneous mixture of Key<?>, Ref<?>, native datastore Key, or
   *          registered entity instances with valid id/parent fields.
   * @return a Map of the asynchronous result. The fetch will be completed when the Map is first accessed.
   */
  ...
  /**
   * Sets the object instance that should be passed on by the base implementation in subsequent actions. You probably
   * don't need to worry about this method; just subclass LoaderWrapper.
   */
