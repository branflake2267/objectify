...
/**
 * The basic options for a typed Query. In addition to adding a few methods that are only available for typed queries,
 * this interface overrides the QueryCommon methods to return the full Query<T>.
 * 
...
public interface Query<T> extends SimpleQuery<T> {
   ...
   * 
   * <p>
   * <strong>The space between the property name and the operator is required.</strong> Filtering a condition of
   * {@code "age>="} will perform an <em>equality</em> test on an entity property exactly named "age>=". You can't
   * create properties like this with Objectify, but you can with the Low-Level API.
   * </p>
   * 
   * <p>
   ...
   * 
   * <p>
   * In addition to filtering on indexed properties, you can filter on @Id properties <strong>if</strong> this query is
   * restricted to a Class<T> which has no @Parent. This is a convenient alias for {@code filterKey()} which builds the
   * key for you.
   * </p>
   * 
   * <p>
   * You can <strong>not</strong> filter on @Parent properties. Use {@code filterKey()} or {@code ancestor()} instead.
   * </p>
   */
   ...
   * 
   * <p>
   * You can sort on @Id properties <strong>if</strong> this query is restricted to a Class<T> which has no @Parent.
   * Note that this is only important for descending sorting; default iteration is key-ascending.
   * </p>
   * 
   * <p>
   * You can <strong>not</strong> sort on @Parent properties.
   * </p>
   */
