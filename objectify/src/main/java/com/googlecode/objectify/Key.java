...
public class Key<T> implements Serializable, Comparable<Key<?>> {
  ...
  public static <T> Key<T> create(com.google.appengine.api.datastore.Key raw) {
    if (raw == null) throw new NullPointerException("Cannot create a Key<?> from a null datastore Key");
  ...
  public static <T> Key<T> create(String webSafeString) {
    if (webSafeString == null) throw new NullPointerException("Cannot create a Key<?> from a null String");
  ...
  /**
   * @return the parent key, or null if there is no parent. Note that the parent could potentially have any type.
   */
  @SuppressWarnings("unchecked")
  public <V> Key<V> getParent() {
    if (this.parent == null && this.raw.getParent() != null) this.parent = new Key<V>(this.raw.getParent());
   ...
   * 
   * @return the topmost parent key, or this object itself if it is the root. Note that the root key could potentially
   *         have any type.
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Call KeyFactory.keyToString() on the underlying Key. You can reconstitute a Key<?> using the constructor that takes
   * a websafe string.
   */
  ...
  /**
   * <p>
   * Determines the kind for a Class, as understood by the datastore. The first class in a hierarchy that has @Entity
   * defines the kind (either explicitly or as that class' simplename).
   * </p>
   * 
  ...
  /**
   * <p>
   * Recursively looks for the @Entity annotation.
   * </p>
   * 
  ...
  private static String getKindHere(Class<?> clazz) {
    ...
    if (ourAnn != null) if (ourAnn.name() != null && ourAnn.name().length() != 0)
      return ourAnn.name();
    else
