...
class CachingTransaction extends TransactionWrapper {
  ...
  /**
   * All futures that have been enlisted in this transaction. In the future, when we can hook into the raw Future<?>, we
   * shouldn't need this - the GAE SDK automatically calls quietGet() on all the enlisted Futures before a transaction
   * commits.
   */
  ...
  public void deferEmptyFromCache(Key key) {
    if (this.deferred == null) this.deferred = new HashSet<Key>();

    this.deferred.add(key);
  }

  /**
   * Adds a Future to our transaction; this Future will be completed before the transaction commits. TODO: remove this
   * method when the GAE SDK provides a way to hook into Futures.
   */
