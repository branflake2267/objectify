...
/**
 * A Future which merges some previously loaded values with the results of another Future that is in progress. It can
 * apply to any key/value pair type; typically it will be for Key/Entity or Key<T>/T
 * 
 * @author Jeff Schnitzer <jeff@infohazard.org>
 */
public class MergeFuture<K, V> implements Future<Map<K, V>> {
  /**
   * Values that we have obtained. Starts with just the preloaded values, then when the pending is complete the values
   * get appended to this collection.
   */
  Map<K, V> loaded;
  ...
  /**
   * @param preloaded is a collection of entities that have already been obtained, say from the memcache. TAKES
   *          OWNERSHIP OF THE MAP OBJECT - it will be modified later.
   * @param pending is a future of entities that will be obtained sometime later, or null if merging is unnecessary and
   *          the preloaded values complete the result.
   */
