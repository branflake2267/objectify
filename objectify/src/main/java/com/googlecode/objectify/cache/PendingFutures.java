 ...
 * 
 * <p>
 * This class maintains a thread local list of all the outstanding Future<?> objects that have pending triggers. When a
 * Future<?> is done and its trigger is executed, it is removed from the list. At various times (anytime an API call is
 * made) the registered futures are checked for doneness and processed.
 * </p>
 * 
 * <p>
 * The AsyncCacheFilter is necessary to guarantee that any pending triggers are processed at the end of the request. A
...
public class PendingFutures {
  ...
  /**
   * Iterate through all pending futures and get() them, forcing any callbacks to be called. This is used only by the
   * AsyncCacheFilter because we don't have a proper hook otherwise.
   */
